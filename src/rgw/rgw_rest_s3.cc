...
{
  if(s && s->root_span)
    s->root_span->SetTag("operation_gateway", "s3");
  span_structure ss;
  #ifdef WITH_JAEGER
    Span span;
...
int RGWGetObj_ObjStore_S3::send_response_data(bufferlist& bl, off_t bl_ofs,off_t bl_len, const Span& parent_span)
{
  parent_span->SetTag("operation_gateway", "s3");
  return RGWGetObj_ObjStore_S3::send_response_data(bl, bl_ofs, bl_len);
}
...
void RGWListBuckets_ObjStore_S3::send_response_end(Span span, const Span& parent_span)
{
  parent_span->SetTag("operation_gateway", "s3");
...
int RGWListBucket_ObjStore_S3::get_params()
{
  ...
  span_structure ss;
    ...
    Span span;
    if(s && !s->stack_span.empty()){
      span = tracer_2.child_span("rgw_rest_s3.cc RGWListBucket_ObjStore_S3::get_common_params", s->stack_span.top());
    }
    else if(s && s->root_span)
      span = tracer_2.child_span("rgw_rest_s3.cc RGWListBucket_ObjStore_S3::get_common_params", s->root_span);
    ss.set_req_state(s);
    ss.set_span(span);
  #endif
...
int RGWPutObj_ObjStore_S3::get_params(Jager_Tracer& tracer, const Span& parent_span)
{
  Span span = tracer.child_span("rgw_rest_s3.cc RGWPutObj_ObjStore_S3::get_params", parent_span);
  if (!s->length)
    return -ERR_LENGTH_REQUIRED;
