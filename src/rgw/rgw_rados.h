...
public:
  ...
  public:
    ...
    struct Write {
      ...
      int _do_write_meta(uint64_t size, uint64_t accounted_size,
                     ...
                     void *index_op, optional_yield y, optional_span* parent_span = nullptr);
      int write_meta(uint64_t size, uint64_t accounted_size,
                     map<std::string, bufferlist>& attrs, optional_yield y, optional_span* parent_span = nullptr);
  ...
  public:
    ...
    public:
      ...
      int complete(int64_t poolid, uint64_t epoch, uint64_t size,
		   ...
		   list<rgw_obj_index_key> *remove_objs, const string *user_data = nullptr, bool appendable = false, optional_span* parent_span = nullptr);
    ...
    protected:
      ...
      int list_objects_ordered(int64_t max,
			       ...
			       bool *is_truncated,
                               optional_yield y, optional_span* parent_span = NULL);
  ...
  int check_bucket_empty(RGWBucketInfo& bucket_info, optional_yield y, optional_span* parent_span = NULL);
   ...
   * Returns 0 on success, -ERR# otherwise.
   */
  int delete_bucket(RGWBucketInfo& bucket_info, RGWObjVersionTracker& objv_tracker, optional_yield y, bool check_empty = true, optional_span* parent_span = NULL);
...
public:
  ...
  int cls_bucket_list_ordered(RGWBucketInfo& bucket_info,
			      ...
			      rgw_obj_index_key *last_entry,
                              optional_yield y,
			      check_filter_t force_check_filter = nullptr, optional_span* parent_span = NULL);
  int cls_bucket_list_unordered(RGWBucketInfo& bucket_info,
				...
				rgw_obj_index_key *last_entry,
                                optional_yield y,
				check_filter_t = nullptr, optional_span* parent_span = NULL);
